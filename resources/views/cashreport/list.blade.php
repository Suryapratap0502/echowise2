                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <div class="page-header-title">
                            <h5 class="m-b-10">Cash Report</h5>
                        </div>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ url('') }}"><i class="feather icon-home"></i></a></li>
                            <li class="breadcrumb-item"><a href="#">Report</a></li>
                            <li class="breadcrumb-item"><a href="#">Cash Report</a></li>
                        </ul>
                    </div>
                </div>
            </div>
                        ...
                        <div class="row align-items-center m-l-0">
                            <div class="col-sm-3">
                                <div class="input-group input-group-sm mb-3">
                                    <span class="input-group-text btn-info">Start Date</span>
                                    <input type="date" class="form-control" aria-label="Small" aria-describedby="inputGroup-sizing-sm" id="startdate" onchange="filter('cash_report_filter')">
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="input-group input-group-sm mb-3">
                                    <span class="input-group-text btn-info"  style="color:white;">End Date</span>
                                    <input type="date" class="form-control" aria-label="Small" aria-describedby="inputGroup-sizing-sm" id="enddate" onchange="filter('cash_report_filter')" >
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="input-group input-group-sm mb-3">
                                    <span class="input-group-text btn-info" style="color:white;">Data Status</span>
                                    <select class="form-select" id="itemname" aria-label="Small" aria-describedby="inputGroup-sizing-sm" onchange="filter('cash_report_filter')">
                                        <option value="" selected>Select</option>
                                        @if (!empty($data_status))
                                            @foreach ($data_status as $value)
                                                <option value="{{ $value->data_status }}">{{ $value->data_status }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-4">
                                <div class="form-group input-group-sm">
                                    ...
                                    <input type="text" class="form-control" id="cash_with" name="cash_with[]"
                                        placeholder="Enter Cash With">
...
<div class="modal fade" id="edit_cash_report" tabindex="-1" role="dialog"
...
<div class="modal fade" id="import" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Import Excel For Cash Report</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                </button>
            </div>
...
<script>
    ...
    $(document).on('submit', '#add_cash', function(ev) {
        $('.error').html('');
...
<script>
    ...
    function add_row() {
        ...
        html += '<div class="mt-2 mb-3" style="border-bottom: 1px solid black"></div>';;
        ...
        html += '<div class="col-sm-4"> <div class="form-group input-group-sm"><label class="form-label" for="Name">Cash With</label><input type="text" class="form-control" id="cash_with" name="cash_with[]" placeholder="Enter Cash With"> <span id="cash_with_error" class="error"></span></div></div>';
        html += '<div class="col-sm-4"> <div class="form-group input-group-sm"><label class="form-label" for="Name">Enter Amount</label><input type="number" class="form-control" id="amount" name="amount[]" placeholder="Enter Amount">  <span id="amount_error" class="error"></span></div></div>';
        html += '<div class="col-sm-4"> <div class="form-group input-group-sm"><label class="form-label" for="Name">Rate</label><input type="text" class="form-control" id="data_status" name="data_status[]"> <span id="status_error" class="error"></span></div> </div>';
        html += '<div class="col-sm-8"> <div class="form-check form-switch" style="float:right;"><div class="d-flex align-items-start gap-3 mt-4" ><button type="button" class="btn btn-danger waves-effect waves-light btn-sm" onclick="removerow(' + rowcount + ')">Less</button></div></div></div><div class="eqress"></div></div></div>';
...
<script>
        $(document).on('submit', '#upload_excel', function(ev) {
            ...
            if (error == false) {
                $.ajax({
                    url: "{{ url('cash_report/importexcel') }} ",
                    type: 'post',
                    data: formData,
                    cache: false,
                    contentType: false,
